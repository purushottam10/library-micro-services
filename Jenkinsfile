   ...
   agent any
        ...
        stages{
          ...
          stage('deploy'){
           steps {
               dir('lib-server'){
                   deploy adapters: [tomcat9(credentialsId[username: 'manager-script' password:'qwerty'], path: '/', url: "https://localhost:8089/lib-server")], onFailure: false, war: 'target/*.war'
               }
                dir('library-dbr'){
                  deploy adapters: [tomcat9(credentialsId[username: 'manager-script' password:'qwerty'], path: '/', url: "https://localhost:8089/library-db")], onFailure: false, war: 'target/*.war'
                }
                dir('member-service'){
